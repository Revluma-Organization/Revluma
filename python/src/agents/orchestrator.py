...
INTENTS = {
    ...
    "ecommerce_strategy":  "General ecommerce strategy/advice question, no store needed.",
...
- chat: greeting, thanks, acknowledgement, casual conversation, "how are you", "okay", "sounds good", emotional response, any social/conversational message. NO analytics needed.
- ecommerce_strategy: general strategy or advice about ecommerce that does NOT require the merchant's specific store data. "How do I reduce cart abandonment?", "What's a good retention strategy?", "How can I improve conversion?", "Tips for recovering abandoned carts", "Best email timing for cart recovery". Use ecommerce expertise, not store data.
...
- business: question about what is happening in THEIR specific store. "Why did MY revenue drop?", "What's happening with MY store?", "Why are MY carts abandoning?", "How is MY store doing?". Requires actual merchant store data.
- analytics: request for specific numbers from THEIR store. "Show me my revenue", "How many carts did I lose yesterday?", "What's my conversion rate?"
- recommendation: asking what THEY should do based on their specific situation. "What should I focus on?", "What should I do next?", "Which of my products should I promote?"
- action: asking Rev to create/send/launch something. "Create a campaign", "Send this email", "Launch this sequence"
- capability: asking what Rev can do or help with. "What can you do?", "How can you help me?", "What are your capabilities?"
...
- "How can I improve my conversion rate?" = ecommerce_strategy (general strategy)
- "What's AOV?" = knowledge (definition)
- "Why is MY revenue down?" = business (store-specific)
- "Show me my revenue" = analytics (store-specific data)
- "Why?" after a business question = followup
- "What should I focus on?" = recommendation (store-specific)
...
def _fallback_classify(message: str, history: list[dict]) -> str:
    ...
    # Ecommerce strategy (how-to questions without "my store")
    strategy = ["how do i", "how can i", "how to", "tips for", "best way to",
                "how should i", "what's a good", "strategies for", "ways to improve",
                "reduce cart", "increase aov", "improve conversion", "recover abandoned",
                "retention strategy", "win back", "re-engage"]
...
def _generate_chat_response(message: str, history: list[dict], memories: list[dict]) -> str:
    ...
    # Handle very common cases instantly — no LLM needed
    instant = {
        ...
        "hi rev":          "Hey. Good to see you. What's on your mind?",
        "good morning":    "Morning! Ready to dig in whenever you are. What's the focus today?",
    ...
    # Check exact matches and punctuation-stripped matches
    if msg in instant:
        return instant[msg]
    ...
    # LLM for anything else conversational
    try:
        api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
        ...
        resp = client.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    ...
                    "If appropriate, invite them to share what they're working on."
                )
            }]
        )
...
def _capability_response() -> str:
    return (
        ...
        "Eventually: help execute recommendations like launching recovery sequences or creating campaigns.\n\n"
...
def _generate_knowledge_response(message: str, history: list[dict], intent: str = "knowledge") -> str:
    """
    General ecommerce knowledge or strategy question.
    No store data needed. Uses haiku for speed.
    Includes conversation context for coherent follow-ups.
    """
    import anthropic, os
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": (
                    f"You are Rev, an ecommerce intelligence assistant built into Revluma.\n"
                    f"You have deep expertise in ecommerce, Shopify, WooCommerce, revenue recovery, "
                    f"retention, customer behavior, and marketing.\n\n"
                    ...
                    + (
                        ...
                        if is_strategy else
                        "This is a knowledge/definition question. Answer directly and accurately. "
...
def _no_store_response(message: str, intent: str) -> str:
    """
    Respond usefully when no store is connected.
    Always give general guidance first. Store connection is mentioned only when
    it materially improves the specific answer.
    """
    import anthropic, os

    msg = message.lower()

    # For recommendation intent — give useful guidance + mention store
    try:
        api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
        ...
        resp = client.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    ...
                    f"The merchant asked: \"{message}\"\n"
                    f"Their store is not connected yet so you don\'t have their specific data.\n"
                    f"Respond helpfully using your ecommerce expertise. Give practical general guidance.\n"
                    f"At the end, mention briefly that you can give a more specific answer once their store is connected.\n"
                    f"Be concise. No em dashes. No corporate language. Sound like a sharp operator."
                )
            }]
        )
        text = "".join(b.text for b in resp.content if getattr(b, "type", "") == "text").strip()
    ...
    return (
        ...
        "For an answer specific to your store, connect Shopify or WooCommerce and I\'ll work from your actual data."
    )
