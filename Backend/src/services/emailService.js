// FIX B-11: Added escapeHtml() and applied it to all user-supplied values in email templates.
// The prior implementation interpolated user data directly into HTML strings, creating XSS
// vectors if user-controlled content contained markup characters.
 ...
 * Handles &, <, >, ", ' — the five characters that must be entity-encoded.
 */
...
class EmailService {
  ...
  async sendAffiliateWelcomeEmail(email, fullName, referralLink, username) {
    try {
      const subject = `Welcome to Revluma Affiliate Program, ${escapeHtml(fullName)}!`;
  ...
  // FIX B-11: fullName is user-supplied — must be escaped before HTML interpolation.
  // OTP code is a 6-digit integer generated by crypto.randomInt — no escaping needed,
  // but escapeHtml is applied as defence-in-depth.
  ...
  // FIX B-11: All user-supplied affiliate profile fields are escaped before interpolation.
  getVettingNotificationTemplate(affiliateProfile) {
    const channels = affiliateProfile.activeChannels || [];
    // FIX B-11: Escape channel platform labels and URLs (user-supplied)
    const channelsHtml = channels.map(c =>
      `<div style="margin: 6px 0;"><span style="color: #888; font-weight: 600;">${escapeHtml(c.platform)}:</span> <span style="color: #ccc;">${escapeHtml(c.url)}</span></div>`
    ).join('');
    ...
    return `
      ...
      <head>
        ...
        <style>
          ...
          .badge { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; color: #a855f7; border: 1px solid #a855f7; }
          ...
          <div class="card">
            ...
            <div class="content">
              <div class="section">
                ...
                <div class="row"><span class="label">Full Name</span><span class="value">${escapeHtml(affiliateProfile.fullName) || 'N/A'}</span></div>
                <div class="row"><span class="label">Email</span><span class="value">${escapeHtml(affiliateProfile.applicantEmail) || 'N/A'}</span></div>
                <div class="row"><span class="label">Username</span><span class="value">${escapeHtml(affiliateProfile.username) || 'N/A'}</span></div>
                <div class="row"><span class="label">Phone</span><span class="value">${escapeHtml(affiliateProfile.phoneNumber) || 'N/A'}</span></div>
                <div class="row"><span class="label">Country</span><span class="value">${escapeHtml(affiliateProfile.country) || 'N/A'}</span></div>
              </div>

              <div class="section">
                <h2>Profile Details</h2>
                <div class="row"><span class="label">Experience Level</span><span class="value">${escapeHtml(affiliateProfile.affiliateExperience) || 'N/A'}</span></div>
                <div class="row"><span class="label">Audience Niche</span><span class="value">${escapeHtml(affiliateProfile.audienceNiche) || 'N/A'}</span></div>
                <div class="row"><span class="label">Audience Size</span><span class="value">${escapeHtml(affiliateProfile.audienceSize) || 'N/A'}</span></div>
                <div class="row"><span class="label">Reason for Joining</span><span class="value">${escapeHtml(affiliateProfile.whyJoin) || 'N/A'}</span></div>
                ${affiliateProfile.referralSource ? `<div class="row"><span class="label">Referral Source</span><span class="value">${escapeHtml(affiliateProfile.referralSource)}</span></div>` : ''}
              </div>
              ...
              <div class="section">
                ...
                <div class="row"><span class="label">User ID</span><span class="value" style="font-family: monospace; font-size: 11px;">${escapeHtml(affiliateProfile.userId) || 'N/A'}</span></div>
                <div class="row"><span class="label">Profile ID</span><span class="value" style="font-family: monospace; font-size: 11px;">${escapeHtml(affiliateProfile.id) || 'N/A'}</span></div>
  ...
  // FIX B-11: fullName and username are user-supplied — escaped before interpolation.
  // referralLink is an internally-generated URL — escaped as defence-in-depth.
  ...
  // FIX B-11: fullName and referredUser.email are user-supplied — escaped before interpolation.
  getConversionNotificationTemplate(fullName, referredUser) {
    const safeName = escapeHtml(fullName);
    const safeReferredEmail = escapeHtml(referredUser?.email || '');
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
        ...
        <div class="container">
          <div class="card">
            ...
            <div class="content">
              ...
              <div class="highlight">
                <p><strong style="color:#fff;">${safeReferredEmail}</strong></p>
  ...
  // FIX B-11: fullName is user-supplied — escaped before interpolation.
  // resetLink is an internally-generated URL and trusted, but escaped as defence-in-depth.
  getPasswordResetTemplate(fullName, resetLink) {
    const safeName = escapeHtml(fullName);
    // resetLink is server-generated (not user input), but we still escape for defence-in-depth.
    // Do NOT encode the href value with escapeHtml — use it verbatim inside the href attribute
    // (browsers do not interpret entities inside href). The display text is static.
    const safeResetLink = (typeof resetLink === 'string') ? resetLink.replace(/"/g, '%22') : '#';
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
          ...
          <div class="card">
            ...
            <div class="content">
              ...
              <p style="text-align: center; margin: 30px 0;">
                <a href="${safeResetLink}" style="display: inline-block; background: #a855f7; color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600;">Reset Password</a>
              </p>
