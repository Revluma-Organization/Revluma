 ...
 * Admin-only endpoints for RAPP management, token generation, and affiliate approval
 */
...
/**
 * POST /api/affiliate/admin/generate-token
 * Generate new RAPP access token
 */
router.post('/generate-token', adminOnly, async (req, res) => {
  try {
    const { description, maxUses = 1, expiresInDays = null } = req.body;
    ...
    const token = await affiliateAuthService.generateAccessToken(
      description,
      maxUses,
      expiresAt,
      req.user.id
    ...
    logger.info('RAPP access token generated by admin', {
      tokenId: token.id,
      adminId: req.user.id,
      description
    });
    ...
    res.status(201).json({
      ...
      token: token.token, // Show plain token only once
      tokenId: token.id,
...
/**
 * POST /api/affiliate/admin/revoke-token/:tokenId
 * Revoke an access token
 */
router.post('/revoke-token/:tokenId', adminOnly, async (req, res) => {
  try {
    const { tokenId } = req.params;
    const { reason } = req.body;

    const token = await affiliateAuthService.revokeAccessToken(
      tokenId,
      req.user.id,
      reason || 'Revoked by admin'
    );

    logger.info('RAPP access token revoked by admin', {
      tokenId,
      adminId: req.user.id,
      reason
    });
    ...
    logger.error('Revoke access token failed', {
      error: error.message,
...
/**
 * GET /api/affiliate/admin/tokens
 * List all access tokens
 */
router.get('/tokens', adminOnly, async (req, res) => {
  try {
    ...
    const [tokens, total] = await Promise.all([
      prisma.rappAccessToken.findMany({
        where,
        select: {
          id: true,
          tokenHash: true,
      ...
      prisma.rappAccessToken.count({ where })
    ]);
    ...
    logger.error('List access tokens failed', {
      error: error.message,
      adminId: req.user.id
    });
    res.status(500).json({ error: 'Internal server error' });
...
/**
 * GET /api/affiliate/admin/token-usage/:tokenId
 * Get usage log for specific token
 */
router.get('/token-usage/:tokenId', adminOnly, async (req, res) => {
  try {
    const { tokenId } = req.params;
    ...
    const [usageLogs, total] = await Promise.all([
      prisma.rappTokenUsageLog.findMany({
        where: { tokenId },
        orderBy: { createdAt: 'desc' },
        skip,
        take: parseInt(limit)
      }),
      prisma.rappTokenUsageLog.count({ where: { tokenId } })
    ]);
    ...
    logger.error('Get token usage failed', {
      error: error.message,
      tokenId: req.params.tokenId,
      adminId: req.user.id
    });
