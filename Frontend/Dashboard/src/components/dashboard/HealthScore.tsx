...
export function HealthScore() {
  const { score, grade, subs } = MOCK.health;
  // Half-circle gauge: pathLen ≈ 157 (per reference)
  const pathLen = 157;
  const offset = pathLen - (score / 100) * pathLen;
  const gradeTone =
  ...
  return (
    <section className="glass-card flex flex-col p-5">
      <header className="mb-4 flex items-center justify-between">
        ...
        <button className="text-t3 transition-colors hover:text-t1" title="Recovery 30% + Deliverability 20% + Subscribers 20% + AOV 15% + Abandonment 15%">
      ...
      <div className="relative mx-auto mb-3 h-[80px] w-[140px]">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-1">
          <div className="display text-[1.6rem] font-extrabold leading-none" style={{ color: gradeTone }}>{score}</div>
          <div className="mt-0.5 text-[0.65rem] font-bold uppercase tracking-wider text-t2">Grade {grade}</div>
        </div>
      </div>
